...
import { QuickFindModal } from "./QuickFindModal/QuickFindModal.tsx";

const MODAL_CONTAINER_ID = "quick-find-search-modal";
...
const NavSearchButton: React.FC = () => (
    ...
    icon={<Search/>}
    label="Search"
    onClick={() => window.dispatchEvent(new CustomEvent("quick-find:open-search"))}
  />
);

const ensureI18nReady = async () => {
  await i18n.loadNamespaces("quick-find");
...
const mountModal = () => {
  ...
  modalRoot.render(
      <I18nextProvider i18n={i18n} defaultNS="quick-find">
          <QuickFindModal/>
      </I18nextProvider>,
  );
};

registry.add("primary-nav-item", "quick-find-search", {
...
export const registerRoutes = async () => {
  ...
  console.debug("%c quick-find is activated", "color: #3c8cba");
};
