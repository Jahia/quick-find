/**
 * Root modal component for the QuickFind search dialog.
 *
 * - Opens/closes via Cmd+K / Ctrl+K / Escape or the `quick-find:open-search` custom event.
...
import { QuickFindPanel } from "../QuickFindPanel/QuickFindPanel.tsx";
import s from "./QuickFindModal.module.css";
...
function logModalDebug(message: string, error: unknown): void {
  ...
  console.debug(`[quick-find][modal] ${message}`, error);
}

export const QuickFindModal = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("quick-find:open-search", handleOpenEvent);
    return () => {
      targets.forEach((targetDoc) =>
        targetDoc.removeEventListener("keydown", handleKeyDown),
      );
      window.removeEventListener("quick-find:open-search", handleOpenEvent);
    };
  }, []);

  if (!apolloClient) {
    console.error(
      "[quick-find] Apollo client is not available — modal cannot render.",
    );
    return null;
  }
  ...
  return (
      ...
      data-quick-find-modal="true"
      onOpenChange={setIsOpen}
      >
          <>
              <div className={s.content}>
                  <ApolloProvider client={apolloClient}>
                      <QuickFindPanel focusOnField onNavigate={() => setIsOpen(false)}/>
                  </ApolloProvider>
              </div>
              <ModalFooter>
                  ...
                  href="https://github.com/Jahia/quick-find/issues"
                  target="_blank"
                  rel="noreferrer"
                              >
                                  https://github.com/Jahia/quick-find/issues
                              </a>
                          </Typography>
                      </div>
                      ...
                      <Typography variant="caption" style={{ opacity: 0.4 }}>
                          {window.contextJsParameters.quickFind?.buildTime ?? ""}
                      </Typography>
            )}
                  </div>
              </ModalFooter>
